...
import { UserState, MusicItem } from '../types';
import { TELEGRAM_CONFIG } from '../config/config';
import axios from 'axios';
import { MUSIC_API } from '../config/config';

// 创建 Telegram Bot 实例
const bot = new TelegramBot(TELEGRAM_CONFIG.TOKEN, { polling: true });

// 全局状态存储
const songIdToUrl: Record<string, string> = {}; // 存储歌曲ID和URL的映射
const userStates: Record<number, UserState> = {}; // 用户状态存储
 ...
 * @param query - 搜索关键词
 */
function handleSearchMode(chatId: number, query: string): void {
 ...
 * @param chatId - 用户的聊天ID
 */
function sendCmsMenu(chatId: number): void {
 ...
 * @param chatId - 用户的聊天ID
 */
function startMusicSearch(chatId: number): void {
 ...
 * @param query - 搜索关键词
 */
async function getMusicList(query: string): Promise<MusicItem[] | void> {
  try {
    const response = await axios.get(`${MUSIC_API.URL}?keyword=${query}`, {
      headers: {
    ...
    // 构建返回的音乐列表
    const musicList = songs.map((song: any) => {
    ...
    if (musicList.length === 0 || musicList[0].text === '未找到相关音乐') {
      if (currentChatId) bot.sendMessage(currentChatId, '未找到相关音乐');
      return [];
    }
 ...
 * @param query - 回调查询
 */
async function downloadMusic(url: string, query: TelegramBot.CallbackQuery): Promise<void> {
  try {
    // 通知用户正在下载
    if (currentChatId) {
 ...
 */
async function CMSUpdate(cmsUrl: string): Promise<void> {
  // 发送增量同步请求
  try {
...
// 初始化 Telegram Bot
export function initTelegramBot(autoGenerateStrm: (command?: string) => Promise<void>, clearVideoFilesCache: () => void): void {
  /**
   * 处理消息事件
   */
  bot.on('message', async (msg) => {
    ...
    // 权限验证
    if (username !== TELEGRAM_CONFIG.AUTHORIZED_USERNAME) {
  ...
  bot.on('callback_query', (query) => {
    ...
    if (currentChatId !== query.message?.chat.id) {
      bot.sendMessage(query.message?.chat.id || 0, '你没有权限');
      return;
    }
...
export function setupTelegramBot(): void {
  ...
  const autoGenerateStrm = async (command?: string) => {
    // 这里将会从控制器调用相应的函数
...
// 导出 Bot 实例和当前聊天ID，以便其他模块使用
export { bot, currentChatId };
